...
const (
	...
	// The efficient_interrupt_approach_b and _d builds both read it with a plain
	// aligned load, which is tear-free and eventually visible on the architectures
	// wazevo compiles for.
	ExecutionContextOffsetModuleClosedPtr Offset = 1248
	// ExecutionContextOffsetInterruptCounter is the offset of `interruptCounter`.
	// Compiled loop back-edges increment it under the efficient_interrupt_approach_b
	// build, and exit to the checkModuleExitCode trampoline every N back-edges
	// regardless of the Closed word. The Go runtime cannot asynchronously preempt
	// goroutines executing wazevo-generated machine code, so that unconditional exit
	// is the only safepoint inside a wasm loop: without it a spinning module would
	// never yield, and stop-the-world GC would livelock waiting on it.
