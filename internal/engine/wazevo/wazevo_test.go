...
// Test_moduleClosedPtrLayout pins the assumption behind execCtx.moduleClosedPtr: that the
// uint64 value of an atomic.Uint64 lives at offset 0 of the struct, and is naturally
// aligned, so that compiled loop back-edges can read it with a single 64-bit load.
