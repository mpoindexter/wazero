...
func TestModule_AssignModuleID(t *testing.T) {
	...
	// The interrupt check interval changes the generated code, so it must change the ID.
	...
	// Without ensureTermination there is no interrupt check to space out, so the interval
	// is not part of the identity.
	require.Equal(t, getID([]byte{1, 2, 3}, nil, false, 0), getID([]byte{1, 2, 3}, nil, false, 32))
}
